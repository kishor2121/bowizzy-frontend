...
import { Plus, Trash2, ChevronDown, RotateCcw, X, Save, Upload } from "lucide-react";
...
export default function CertificationDetailsForm({
    ...
    const initialCertificates: Certificate[] = initialData.certificates && initialData.certificates.length > 0
    ...
    useEffect(() => {
        certificates.forEach(c => { initialCertsRef.current[c.id] = { ...c }; });
    }, []);
    ...
    useEffect(() => {
        ...
        certificates.forEach(current => {
            ...
            if (current.certificateType !== (initial?.certificateType || "")) changedFields.push('certificateType');
            if (current.certificateTitle !== (initial?.certificateTitle || "")) changedFields.push('certificateTitle');
            if (current.domain !== (initial?.domain || "")) changedFields.push('domain');
            if (current.certificateProvidedBy !== (initial?.certificateProvidedBy || "")) changedFields.push('certificateProvidedBy');
            ...
            if (current.uploadedFile !== initial?.uploadedFile) changedFields.push('fileUpload');
            else if (current.uploadedFileUrl !== (initial?.uploadedFileUrl || "")) changedFields.push('uploadedFileUrl');
    ...
    const resetCertificate = (index: number) => {
        ...
        updated[index] = {
            ...
            uploadedFile: null, 
            uploadedFileName: initial?.uploadedFileName || (initial?.uploadedFileUrl ? initial.uploadedFileUrl.split('/').pop() : ""),
        ...
        setCertChanges(prev => { const newChanges = { ...prev }; delete newChanges[updated[index].id]; return newChanges; });
        setErrors((prev) => {
        ...
        setCertFeedback(prev => { const newFeedback = { ...prev }; delete newFeedback[updated[index].id]; return newFeedback; });
    };
    ...
    const removeCertificate = async (index: number) => {
        ...
        if (cert.certificate_id) {
            try {
                ...
                setTimeout(() => setCertFeedback(prev => { const updated = { ...prev }; delete updated[cert.id]; return updated; }), 3000);
                return;
            }
        }
        ...
        setCertChanges(prev => { const updated = { ...prev }; delete updated[id]; return updated; });
    };
    ...
    const handleSaveCertificate = async (cert: Certificate, index: number) => {
        ...
        if (!changes || changes.length === 0) {
            ...
            setTimeout(() => setCertFeedback(prev => { const updated = { ...prev }; delete updated[certId]; return updated; }), 3000);
            return;
        }
        ...
        try {
            ...
            if (finalCertData.certificate_id) {
                updatedCert = { 
                    ...
                    uploadedFileName: finalCertData.file_url ? finalCertData.file_url.split('/').pop() : cert.uploadedFileName,
                };
                setCertFeedback(prev => ({ ...prev, [certId]: isNew ? "Saved successfully!" : "Updated successfully!" }));
            }
            ...
            setCertChanges(prev => { const updated = { ...prev }; delete updated[certId]; return updated; });
            setTimeout(() => setCertFeedback(prev => { const updated = { ...prev }; delete updated[certId]; return updated; }), 3000);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (hasUnsavedChanges) {
            Object.keys(certChanges).forEach(id => {
                setCertFeedback(prev => ({ ...prev, [id]: "Please save your certificate changes before proceeding" }));
                setTimeout(() => setCertFeedback(prev => { const updated = { ...prev }; delete updated[id]; return updated; }), 3000);
            });
            return;
        }
        
        // Filter out completely empty certificates before sending to next step
        const validCertificates = certificates.filter(c => c.certificateTitle || c.certificate_id);
